...
| 📂 **标题折叠** | 支持 H1/H2/H3 标题折叠，快速收起展开内容区块    |
| 🔒 **隐私模式** | 标记敏感内容为隐私，隐私模式下自动隐藏          |
| ↕️ **拖拽排序** | 拖拽标题可带动下方内容整体移动                  |
| 📋 **一键复制** | 智能分类复制日志内容，自动区分完成/未完成工作   |
...
启动成功后，访问 http://localhost:3001 即可使用。
...
- **一键复制**: 点击标题栏 "复制" 按钮，自动将日志内容转换为分类列表格式
- **标题折叠**: 点击标题左侧的 ▼/▶ 按钮，可折叠/展开标题下方的内容

#### 标题折叠功能

支持对 H1/H2/H3 三级标题进行折叠操作：

- **H1 折叠**: 隐藏到下一个 H1 之前的所有内容
- **H2 折叠**: 隐藏到下一个 H1 或 H2 之前的所有内容
- **H3 折叠**: 隐藏到下一个 H1、H2 或 H3 之前的所有内容

折叠状态会自动保存，方便下次打开时保持视图状态。

#### 隐私模式功能

保护敏感工作内容，生成周报时自动过滤：

- **标记隐私**: 点击标题左侧的眼睛图标（👁）可标记/取消隐私
- **切换模式**: 点击顶部工具栏的眼睛图标切换阅读模式（正常/隐私）
- **隐私模式**: 隐私模式下，被标记为隐私的标题及其下方内容不显示
- **周报过滤**: 生成 AI 周报时，隐私内容会被自动排除

#### 拖拽排序功能

支持通过拖拽调整内容顺序：

- **拖拽手柄**: 使用左侧的拖拽手柄（⋮⋮）来拖动内容
- **标题拖拽**: 拖拽标题时会自动带上下方所有内容一起移动
- **层级规则**:
  - H1 拖拽：带上其下所有 H2、H3、段落、待办
  - H2 拖拽：带上其下所有 H3、段落、待办（直到下一个 H1 或 H2）
  - H3 拖拽：带上其下所有段落、待办（直到下一个 H1、H2 或 H3）

#### 一键复制功能

点击复制按钮后，日志内容会自动转换为以下格式：
